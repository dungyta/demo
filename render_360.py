...
def set_cam(dist=1.0, elev=0.0, azim=30, degrees=True):
    ...
    cameras = FoVPerspectiveCameras(R=R, T=T, device=device)  # ← Thêm device=device
    return cameras
...
def load_mesh(path:str):
    ...
    texture_rgb = torch.ones_like(vertices, device=device)
    texture_rgb = texture_rgb * torch.tensor([1.0, 1.0, 1], device=device)
...
def render_360(path:str, num_frames=12, dist=3.0, elev=0.0):
    ...
    for a in angles:
        ...
        i = image[0, ..., :3].cpu().numpy()  # ← Chuyển về CPU để save
